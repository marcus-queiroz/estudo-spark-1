...
# Renomear as colunas do DataFrame de clientes atualizados para evitar conflito
clientes_atualizados_df = clientes_atualizados_df.select(
    [col(c).alias(f"novo_{c}") if c != "cliente_id" else col(c) for c in clientes_atualizados_df.columns]
)
...
condicao_mudanca = (clientes_atualizados_df.cliente_id == clientes_delta_df.cliente_id) & \
                   (clientes_atualizados_df.novo_cidade != clientes_delta_df.cidade) & \
...
clientes_historico_atualizado_df = clientes_delta_df.join(clientes_atualizados_df, "cliente_id", "left") \
    .withColumn("ativo_atualizado", when(condicao_mudanca, lit(False)).otherwise(clientes_delta_df.ativo)) \
    .withColumn("data_fim_atualizado", when(condicao_mudanca, current_date()).otherwise(clientes_delta_df.data_fim)) \
    .drop("ativo", "data_fim") \
    .withColumnRenamed("ativo_atualizado", "ativo") \
    .withColumnRenamed("data_fim_atualizado", "data_fim")
...
# Garantir que os dois DataFrames tenham o mesmo esquema
...
clientes_novos_atualizados_df = clientes_novos_atualizados_df.select(
    "cliente_id", "novo_nome", "novo_cidade", "novo_data_cadastro", "ativo", "data_inicio", "data_fim"
).withColumnRenamed("novo_nome", "nome") \
 .withColumnRenamed("novo_cidade", "cidade") \
 .withColumnRenamed("novo_data_cadastro", "data_cadastro")
